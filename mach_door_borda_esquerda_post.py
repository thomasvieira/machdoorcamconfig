...
TOOLTIP = """
Gera g-code para usinagem da borda do lado da dobradiça.
import mach_door_borda_esquerda
mach_door_borda_esquerda.export(object, "/path/to/file.ncc")
"""
...
OUTPUT_TOOL_CHANGE = (
    True  # default output M6 tool changes
)
DRILL_RETRACT_MODE = (
...
parser = argparse.ArgumentParser(prog="mach_door_borda_esquerda", add_help=False)
...
ParamOffset5 = "MachDoorCAMConfigOffset5"
...
def getDistPivotCone0Graus():
    """Retorna um float com a distancia do centro de rotação do eixo C até o cone.
    """
    pref = preferences()
    return pref.GetFloat(ParamOffset1, 0.01)

def getDistPivotCone90Graus():
    pref = preferences()
    return pref.GetFloat(ParamOffset2, 0.01)

def getDistPivotConeMenos90Graus():
    pref = preferences()
    return pref.GetFloat(ParamOffset3, 0.01)

def getPosCFechadura():
    pref = preferences()
    return pref.GetFloat(ParamOffset4, 0.01)

def getPosCDobradica():
    pref = preferences()
    return pref.GetFloat(ParamOffset5, 0.01)

def getToolLength(tool):
    if tool == 0:
        return 0
    pref = preferences()
    return pref.GetFloat("MachDoorCAMConfigT" + str(tool) + "Length", 0.000)
...
def export(objectslist, filename, argstring):
    ...
    # write header
    if OUTPUT_HEADER:
        gcode += linenumber() + "(Exportado do FreeCAD)\n"
        gcode += linenumber() + "(Pós Processador: Mach Door Borda Esquerda)\n"
        gcode += linenumber() + "(Gerado em: " + str(datetime.datetime.now()) + ")\n"
    ...
    # Write the preamble
    if OUTPUT_COMMENTS:
        gcode += linenumber() + "(Seleciona plano, modo de posicionamento e unidade  de medida)\n"
    ...
    for obj in objectslist:
        ...
        if OUTPUT_COMMENTS:
            gcode += linenumber() + "(Inicio da operação " + obj.Label + ")\n"
        ...
        # do the post_op
        if OUTPUT_COMMENTS:
            gcode += linenumber() + "(Fim da operação " + obj.Label + ")\n"
    ...
    if OUTPUT_BCNC:
        ...
        gcode += linenumber() + "(Block-enable: 1)\n"
    if OUTPUT_COMMENTS:
        gcode += linenumber() + "(Operações finais)\n"
...
def parse(pathobj):
    ...
    if hasattr(pathobj, "Group"):  # We have a compound or project.
        ...
        for c in PathUtils.getPathWithPlacement(pathobj).Commands:
            ...
            if command in ("G1", "G01"):
                outstring.append("\nM706\n")
                if primeiroG1aposG0:
                    primeiroG1aposG0 = False
                    outstring.append("G04 P1000\n")
            ...
            for param in params:
                if param in c.Parameters:
                    if param == "F":
                        ...
                        CURRENT_TOOL = int(c.Parameters[param])
                        if int(c.Parameters[param]) <= 10 :
                            print(command + " " + param + str(int(c.Parameters[param])))
                            outstring.append("\nG00 C" + str(getPosCDobradica()))
                            deslocamento = getDistPivotConeMenos90Graus() + getToolLength(CURRENT_TOOL) + 1000
                            deslocamento = deslocamento * -1
                            outstring.append("\nG00 X" + format(deslocamento, precision_string))
                        ...
                        correctedDistPivotCone = getDistPivotCone0Graus() * -1
                        correctedDistPivotCone = correctedDistPivotCone + 1.12
                        c.Parameters[param] = correctedDistPivotCone + c.Parameters[param]
                        ...
                        c.Parameters[param] = c.Parameters[param] + getDistPivotConeMenos90Graus() + getToolLength(CURRENT_TOOL)
                        c.Parameters[param] = c.Parameters[param] * -1

                        pos = Units.Quantity(c.Parameters[param], FreeCAD.Units.Length)

                        outstring.append(
                            "X" + format(float(pos.getValueAs(UNIT_FORMAT)), precision_string)
                        )
                    elif param in ["I"]:
                        pos = Units.Quantity(c.Parameters[param], FreeCAD.Units.Length)
                        #pos = pos * -1
                        ...
                        outstring.append(
                            "I" + format(float(pos.getValueAs(UNIT_FORMAT)), precision_string)
                        )
                    else:  # [X, Y, Z, U, V, W, J, R, Q] (Conversion eventuelle mm/inches)
                        pos = Units.Quantity(c.Parameters[param], FreeCAD.Units.Length)
                        outstring.append(
                            param + format(float(pos.getValueAs(UNIT_FORMAT)), precision_string)
                        )
